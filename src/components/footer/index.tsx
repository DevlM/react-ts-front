...
const LinksList: FC<LinkListProps> = (props) => (
  <ul className="space-y-2.5">
    {props.links.map((link, i) => (
      <li key={link.link + '-' + i}>
        <Link
          className="font-outfit font-light tracking-wider text-muted"
          href={link.link}
        >
          {link.title}
        </Link>
  ...
  return (
    <div
      className={cn(
        ...
        'py-10',
        isMobile && 'border-t border-t-accent'
      )}
    >
      <div className="space-y-3">
        <h4 className="text-societe-primary">{props.title}</h4>
...
export default function Footer() {
  return (
    <footer>
      <div className={cn('space-y-10', 'pt-25 pb-15 px-5')}>
        ...
        >
          ...
          <LinkContainer
            title="Societe2"
            links={[
              { title: 'Histoire de Societe2', link: '/' },
