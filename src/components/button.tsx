...
const btnVariants = {
  default: cn(
    'backdrop-blur-xs border-background hover:text-societe-primary hover:border-societe-primary'
  ),
  secondary: cn(
    'text-societe-primary border-societe-primary hover:bg-societe-primary hover:text-background'
  ),
  tertiary: cn(
    'text-background bg-societe-primary border-societe-primary hover:bg-societe-tertiary hover:text-background'
  ),
};
...
const LinkForBtn: FC<ButtonLinkProps> = (props) => {
  return (
    <Link
      href={props.href}
      className={cn(
        'font-semibold whitespace-nowrap w-full h-full',
        btnPadding
      )}
    >
