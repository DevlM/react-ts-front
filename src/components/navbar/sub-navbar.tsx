...
const subNavbarVariants: Record<IAppVariant, string> = {
  societe: cn('bg-societe-primary'),
  societe2: 'bg-societe2-primary',
};
...
const subNavbarLinksVariants: Record<IAppVariant, ILink[]> = {
  ...
  societe2: [
    ...
    { name: 'Histoire de Societe2', href: '/' },
  ],
};
  ...
  return (
    <nav
      className={cn(
        ...
        isNavbarVisible
          ? 'translate-y-0'
          : '-translate-y-12.5 md:-translate-y-19'
      )}
    >
      ...
      <div className="flex items-center gap-5 ml-auto text-background">
        <Link variant="navbar" href={subNavbarLinksVariants[variant][0].href}>
          {subNavbarLinksVariants[variant][0].name}
        </Link>
        {isMobile ? (
          <BarsListIcon
            onClick={toggleSideBar.bind(null, 'right')}
            className="rotate-180 h-[40px]"
          />
        ) : (
          subNavbarLinksVariants[variant].slice(1).map((link, index) => (
            <Link variant="navbar" key={index} href={link.href}>
              {link.name}
            </Link>
          ))
        )}
